...
export default function AgainstAi(props) {
      ...
      if (aiBoard > userBoard) {
        ...
        if (props.userData) {
          const { data } = await axios.post(
            `https://algoracer-backend-5bed1a87253c.herokuapp.com/leaderboard/add/${props.userData.id}`,
            {
              name:
                boardType === "bubble"
                  ? "Bubble Sort"
          ...
          await axios.post(
            `https://algoracer-backend-5bed1a87253c.herokuapp.com/user/stats/${props.userData.id}`,
            {
              difficulty: diff,
              time_taken: timer,
              got_score: userScore,
  ...
  useEffect(() => {
    ...
    if (userBoard === boardsToSolve.current.length - 2 && userBoard > 7) {
      let variation =
        difficulty === "beginner" ? 0 : difficulty === "normal" ? 1 : 2;
      if (boardType === "bubble") {
    ...
    if (userBoard !== 0) {
      ...
      aiInterval.current =
        aiInterval.current - aiInterval.current * (useSpeed / 100);
      setSpeed(useSpeed);
    }
  }, [userBoard]);
  ...
  const setUpLists = useCallback(() => {
    ...
    let variation =
      difficulty === "beginner" ? 0 : difficulty === "normal" ? 2 : 3;
    if (boardType === "bubble") {
      let btdslst = []; // boards to be solved list
      for (let k = 0; k < 10; k++) {
        let slst = [];
  ...
  function getRandomString() {
    let random_strings = [
      "The bot has solved all the boards!",
      "The bot has bested you!",
      "Kneel before your bot god!",
    ...
    <div>
      <div className={styles.main}>
        ...
        <div className={styles.top_btns}>
          {started === true && sortedLists && sortedLists.length !== 0 ? (
        ...
        {!showTutorial ? null : (
          <div className={styles.tutorial}>
            <button
              className={styles.close_tut_btn}
              onClick={() => handleHelp()}
            >
              Close
            </button>
        ...
        {aiSolved === true ? (
          <div className={styles.end_container}>
            ...
            <button className={styles.back_btn} onClick={() => handleCancel()}>
              Restart
            </button>
          </div>
        ) : null}
        ...
        {started === true && sortedLists && sortedLists.length !== 0 ? (
          <div> Speedup: {speed.toFixed(2)} % </div>
        ) : null}
        {started === true && sortedLists && sortedLists.length !== 0 ? (
          <div> Current Speed: {aiInterval.current.toFixed(2)} s </div>
        ) : null}
        {started === true && sortedLists && sortedLists.length !== 0 ? (
          ...
          <div className={styles.boards_container}>
            <div className={styles.board}>
              ...
              <CompeteBoardEndless
                ...
                roomId={"none"}
                solvedBoard={[]}
                changeScore={setUserScore}
                changeBoard={setUserBoard}
                changeSolved={setUserSolved}
                boardsToBeSolved={boardsToSolve}
              />
            </div>
            <div className={styles.board}>
              ...
              <AiBoardEndless
                ...
                randomList={randomList}
                sortedLists={sortedLists}
                solvedBoard={aiSolvedBoard}
                aiInterval={aiInterval}
                intervalIndex={intervalIndex}
                changeScore={setAiScore}
                changeBoard={setAiBoard}
                changeSolved={setAiSolved}
                boardsToBeSolved={boardsToSolve}
              />
            </div>
          </div>
